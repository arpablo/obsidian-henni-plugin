...
import { DEFAULT_SETTINGS, ImageNoteSettingTab, type HenniPluginSettings, type NoteNamingConvention } from './settings';
...
export default class HenniPlugin extends Plugin {
    ...
    private isAutoCreateEnabled(kind: MediaKind): boolean {
        if (kind === 'image') return !!this.settings.autoCreateImages;
        if (kind === 'pdf') return !!this.settings.autoCreatePdfs;
        return !!this.settings.autoCreateOther;
    }
    ...
    private computePrimaryNotePath(file: TFile, kind: MediaKind, folder: string): { baseName: string; notePath: string } {
        ...
        const naming = this.settings.noteNamingConvention;
    ...
    private async appendIncomingLinks(file: TFile): Promise<void> {
        const target = this.getReferencedSource(file);
        if (!target || target.type !== 'file') {
            new Notice('Referenced file not found.');
            return;
        }
        const links = this.app.metadataCache.resolvedLinks;
        const incoming: string[] = [];
        for (const [sourcePath, targets] of Object.entries(links)) {
            if (targets && Object.prototype.hasOwnProperty.call(targets, target.value.path)) {
                incoming.push(sourcePath);
            }
        }
        if (incoming.length === 0) {
            new Notice('No incoming links found for referenced file.');
            return;
        }
        const unique = Array.from(new Set(incoming));
        const wikiLinks = unique.map(path => `- [[${path}]]`).join('\n');
        await this.app.vault.process(file, data => {
            const trimmed = data.endsWith('\n') ? data : data + '\n';
            return `${trimmed}\n${wikiLinks}\n`;
        });
        new Notice('Incoming links appended to note.');
    }

    public isMediaNote(file: TFile): boolean {
        ...
        const ext = target.value.extension?.toLowerCase();
        if (!ext) return false;
        if (this.settings.imageExtensions.includes(ext)) return true;
        if (this.settings.pdfExtensions.includes(ext)) return true;
        if (this.settings.otherExtensions.includes(ext)) return true;
        return false;
    }
    ...
    async onload() {
        ...
        this.addCommand({
            id: 'add-incoming-links-for-referenced-file',
            name: 'Add incoming links of referenced file',
            checkCallback: (checking) => {
                const file = this.app.workspace.getActiveFile();
                if (!file || file.extension?.toLowerCase() !== 'md') {
                    return false;
                ...
                void this.appendIncomingLinks(file);
                return true;
            },
        });
        ...
        this.registerEvent(this.app.vault.on('create', async (file) => {
            ...
            const kind = this.resolveKind(file);
            if (!kind) return;
            if (!this.isAutoCreateEnabled(kind)) return;
            if (!this.isSourceAllowed(file, kind)) return;
            try {
                const folderSetting = this.getTargetFolder(kind) ?? '';
                if (kind === 'other' && !folderSetting.trim()) {
                    return;
                }
                await this.processMedia(file, kind, folderSetting);
            } catch (e) {
            ...
            if (abstract instanceof TFile) {
                ...
                if (file.extension?.toLowerCase() === 'md') {
                    ...
                    if (reference) {
                        ...
                        if (this.isMediaNote(file)) {
                            actions.push({
                                title: 'Add incoming links of referenced file',
                                handler: async () => {
                                    await this.appendIncomingLinks(file);
                                },
                            });
                        }
                    }
    ...
    async loadSettings() {
        ...
        const ensureBoolean = (value: any, fallback: boolean) => typeof value === 'boolean' ? value : fallback;
        const legacyAutoCreate = typeof (stored as any)?.autoCreateOnFileAdd === 'boolean' ? (stored as any).autoCreateOnFileAdd : undefined;
        this.settings.autoCreateImages = ensureBoolean((stored as any)?.autoCreateImages, legacyAutoCreate ?? DEFAULT_SETTINGS.autoCreateImages);
        this.settings.autoCreatePdfs = ensureBoolean((stored as any)?.autoCreatePdfs, legacyAutoCreate ?? DEFAULT_SETTINGS.autoCreatePdfs);
        this.settings.autoCreateOther = ensureBoolean((stored as any)?.autoCreateOther, legacyAutoCreate ?? DEFAULT_SETTINGS.autoCreateOther);
        const validNaming: NoteNamingConvention[] = ['prefix', 'suffix', 'asset-basename'];
        const storedNaming = (stored as any)?.noteNamingConvention;
        if (typeof storedNaming === 'string' && (validNaming as string[]).includes(storedNaming)) {
            this.settings.noteNamingConvention = storedNaming as NoteNamingConvention;
        } else if (typeof (stored as any)?.useSuffix === 'boolean') {
            this.settings.noteNamingConvention = (stored as any).useSuffix ? 'suffix' : 'prefix';
        } else {
            this.settings.noteNamingConvention = DEFAULT_SETTINGS.noteNamingConvention;
        }
        delete (this.settings as any).useSuffix;
        delete (this.settings as any).autoCreateOnFileAdd;
